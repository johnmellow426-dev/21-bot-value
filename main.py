...
# ПРОВЕРКА КАНАЛОВ
# ============================================================
async def check_channels(application: Application) -> bool:
    ...
    try:
        ...
        if source_member.status in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
...
async def try_predict(application: Application, n: int):
    ...
    try:
        ...
        sent = await application.bot.send_message(TARGET, text)
...
async def check_pending(application: Application, n: int):
...
async def reset_daily(application: Application):
...
async def start_command(update: Update, context):
    ...
    await update.message.reply_text("Бот запущен! Ожидаю игры...")
...
async def channel_post_handler(update: Update, context):
    ...
    n = int(m.group(1))
    cards_text = m.group(2).replace('️','')
    ...
    if n == 1440:
        ...
        await reset_daily(context.application)
    
    print(f"   🎯 Пытаюсь сделать прогноз для игры #{n}...")
    await try_predict(context.application, n)
    
    print(f"   🔎 Проверяю активные прогнозы...")
    await check_pending(context.application, n)
...
async def post_init(application: Application):
    ...
    if not await check_channels(application):
        print("\n❌ ПРОВЕРКА КАНАЛОВ НЕ ПРОЙДЕНА.")
        print("   Бот будет запущен, но может не работать корректно.")

def main():
    """Главная функция запуска бота"""
    print("\n" + "="*60)
    print("🚀 ЗАПУСК БОТА (PTB)")
    print("="*60)
